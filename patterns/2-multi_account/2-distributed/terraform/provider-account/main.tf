...
# --- patterns/2-multi_account/2-distributed/terraform/provider-account/main.tf ---
...
# ---------- VPC LATTICE ACCESS LOGGING (SERVICE SCOPE - THIS ACCOUNT OWNS THE SERVICE) ----------
...
resource "aws_vpclattice_access_log_subscription" "service_access_logs" {
  resource_identifier = module.vpclattice_lambda_service.services["lambdaservice"].attributes.arn
...
# RAM share (re-use the RAM resource share created for the VPC Lattice service)
